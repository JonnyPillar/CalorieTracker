...
namespace CalorieTracker.Controllers
{
    public class DashboardController : Controller
    {
        ...
        private  List<tbl_user_metric> userInfoList = new List<tbl_user_metric>();
        ...
        public ActionResult Index()
        {
            ...
            {
                var query = "SELECT * FROM tbl_user_metric WHERE user_information_metric_id = ? ORDER BY SUBSTR(user_information_timestamp,5,4) DESC, SUBSTR(user_information_timestamp,3,2) DESC, SUBSTR(user_information_timestamp,1,2) DESC, SUBSTR(user_information_timestamp,9,2) DESC, SUBSTR(user_information_timestamp,11,2) DESC";
                tbl_user_metric temp = db.tbl_user_metric.SqlQuery(query, item.user_metric_id).First();
